...
from metrix.metrics.tm_port.utilities.checks import (
    _check_classification_inputs,
    _input_format_classification,
    _input_squeeze,
)
from metrix.metrics.tm_port.utilities.enums import (
    AverageMethod,
    DataType,
    MDMCAverageMethod,
)
...
def _accuracy_compute(
    ...
    if (
        mode == DataType.BINARY and average in simple_average
...
def accuracy(
    ...
    Example:
        >>> import torch
        >>> from metrix.metricsfunctional import accuracy
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> preds = torch.tensor([0, 2, 1, 3])
