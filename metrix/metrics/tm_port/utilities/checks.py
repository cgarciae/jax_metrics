...
from metrix.metrics.tm_port.utilities.data import select_topk, to_onehot
...
def _basic_input_validation(
    preds: Tensor, target: Tensor, threshold: float, multiclass: Optional[bool]
) -> None:
...
def _check_shape_and_type_consistency(
    ...
    if preds.ndim == target.ndim:
        ...
        implied_classes = np.prod(preds[0].shape)
    ...
    * preds and target are of shape ``(N, ...)`` both are integers (multi-dimensional
    ...
    if case in (DataType.MULTICLASS, DataType.MULTIDIM_MULTICLASS) or multiclass:
        if _is_floating_point(preds):
            ...
            #     else jnp.maximum(preds.max(), target.max()) + 1
            # )
    ...
    return preds.astype(jnp.int64), target.astype(jnp.int64), case
