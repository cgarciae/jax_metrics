...
from metrix import types

Tensor = jnp.ndarray
tensor = jnp.array

from metrix.metrics.tm_port.classification.stat_scores import StatScores
from metrix.metrics.tm_port.functional.classification.accuracy import (
    _accuracy_compute,
    _accuracy_update,
...
from metrix.metrics.tm_port.utilities.enums import DataType
...
class Accuracy(StatScores):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:  # type: ignore
        ...
        if self.mode is None:
            # NOTE: this results in an additional recompilation after the second first step.
            # this could be avoided if the user explicitly passes mode.
            self.mode = mode
        elif self.mode != mode:
        ...
        if self.subset_accuracy:
            ...
            if self.reduce != "samples" and self.mdmc_reduce != "samplewise":
                ...
                assert isinstance(self.tp, list)
                assert isinstance(self.fp, list)
                assert isinstance(self.tn, list)
                assert isinstance(self.fn, list)
                self.tp.append(tp)
                self.fp.append(fp)
                self.tn.append(tn)
                self.fn.append(fn)
    ...
    def compute(self) -> Tensor:
        ...
        return _accuracy_compute(
            tp, fp, tn, fn, self.average, self.mdmc_reduce, self.mode
        )
