...
from metrix.losses.loss import Loss, Reduction
...
class MeanAbsolutePercentageError(Loss):
    ...
    mape = mtx.losses.MeanAbsolutePercentageError(reduction=mtx.losses.Reduction.SUM)

    assert np.isclose(mape(target, preds), 5.6, rtol=0.01)

    # Using 'none' reduction type.
    mape = mtx.losses.MeanAbsolutePercentageError(reduction=mtx.losses.Reduction.NONE)
    ...
    model = elegy.Model(
        module_fn,
        loss=mtx.losses.MeanAbsolutePercentageError(),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Arguments:
            reduction: (Optional) Type of `mtx.losses.Reduction` to apply to
