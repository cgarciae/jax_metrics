...
class Loss(ABC):
    ...
    * `call()`: Contains the logic for loss calculation.
    ...
    def index_into(self, **kwargs: types.IndexLike) -> "IndexedLoss":
        ...
        ```python
        losses = mtx.Losses([
            mtx.losses.Crossentropy().index_into(target=["a"]),
            mtx.losses.Crossentropy().index_into(target=["b"]),
        ]).reset()
    ...
    def map_arg(self, **kwargs: str) -> "MapArgsLoss":
        ...
        ```python
        crossentropy_loss = mtx.losses.Crossentropy().map_arg(target="y_true", preds="y_pred")
        ...
